...
class Cookies {
  ...
  addListeners(){
    let acceptAllButton = document.getElementsByClassName("cookie-accept")
    let acceptSelectedButton = document.getElementsByClassName("cookie-accept-selection")
    let declineButton = document.getElementsByClassName("cookie-deny")
    let showfurtherSettings = document.getElementById("show-cookie-further-settings")
    let hidefurtherSettings = document.getElementById("hide-cookie-further-settings")

    acceptAllButton[0].addEventListener("click", () => { this.setCookie(true, true, true, true) })
    acceptSelectedButton[0].addEventListener("click", () => { this.setCookie(this.consent, this.statistics, this.external, this.functional) })
