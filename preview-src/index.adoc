...
the link:https://github.com/plentymarkets/plugin-hello-world[github repository] and try to figure yourself out what the code means. Otherwise, stay tuned and keep following the next 3 steps!


=== First File

The first thing that we are going to do is create a json, which contains all needed information about our plugin, that we need.
[source,json]
----
{
      <1>"name":"HelloWorld",
      ...
      <4>"author":"plentysystems AG",
      <5>"type":"template",
      <6>"serviceProvider":"HelloWorld\\Providers\\HelloWorldServiceProvider"
}
----
